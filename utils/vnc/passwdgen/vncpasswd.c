...
//codigo para nuestro vncpasswd
printf("VNC file password generator\n\n");
if(argc !=3){
usage(argv);
exit(1);
}
//Copiando el directorio a la variable
strcpy(passwdFile, argv[2]);
//Copiando el password a la variable.
if(strlen(argv[1])>8){
 fprintf(stderr,"Password too long");
 exit(1);
}
strcpy(passwd1,argv[1]);

if (!vncEncryptAndStorePasswd2(passwd1, passwd1, passwdFile)) {
