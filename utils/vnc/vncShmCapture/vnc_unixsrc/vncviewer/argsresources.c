...
#if VNC_CAPTURE
   {"shmCapture", "shmCapture", XtRBool, sizeof(Bool),
   XtOffsetOf(AppData, shmCapture), XtRImmediate, (XtPointer) False},

  {"idShMemory", "idShMemory", XtRInt, sizeof(int),
   XtOffsetOf(AppData, idShMemory), XtRImmediate, (XtPointer) -1},
  {"sizeShMemory", "sizeShMemory", XtRInt, sizeof(int),
   XtOffsetOf(AppData, sizeShMemory), XtRImmediate, (XtPointer) -1},
#endif
...
#if VNC_CAPTURE
  {"-shmCapture",    "*shmCapture",         XrmoptionNoArg,  "True"},
  {"-idShMemory",    "*idShMemory",         XrmoptionSepArg,  0},
  {"-sizeShMemory",  "*sizeShMemory",       XrmoptionSepArg,  }
#endif
};
...
usage(void)
{
  fprintf(stderr,
      ...
      "        -shmCapture (Enable the shared memory feature)\n"
      "        -idShMemory (id for the Shared Memory)\n"
      "        -sizeShMemory (size for the Shared Memory in bytes)"
#endif
	  "\n"
...
{
  int i,j=0; // Gabriel: poner j en la linea siguiente
  ...
  */
  ...
  /* parser should be changed!
     this is an ugly method for backwards compatibility */

  for (i = strlen(vncServerName)-1; vncServerName[i] != ':' && i >= 0; i--) j++;
  /* ----------------------------------------------------------------
    -----------------------------------------------------------------
  ...
  */
  ...
  if (vncServerName[i] == ':') {
    ...
    strncpy(vncServerHost, vncServerName, strlen(vncServerName)-j-1);
    if(strlen(vncServerHost)==0){
    strcat(vncServerHost,"localhost");
    }

  }
  else {
    vncServerPort = 0;
    strncpy(vncServerHost, vncServerName, strlen(vncServerName));
  }
