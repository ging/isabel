...
/* Compilar con el codigo de capturar las imagenes */
#define VNC_CAPTURE 1
...
extern void vnc_capture_init(XImage *theimage);  /* initialize the image capture */
extern void vnc_capture_cleanup();  /* finish and cleanup the image capture */
extern void myxfillrec(Display *dpy,GC gc,int x,int y,unsigned int w,unsigned int h); /* call XFillRectangle to the pixmap */
extern void myxcparea(Display *display,GC gc,int src_x,int src_y,int width,int height,int dest_x, int dest_y); /* call XCopyArea with the pixmap as parameter */
/* Function to handle the soft mouse */
extern void myxcpareaTo(Display *display,Drawable dstDraw,GC gc,int src_x,int src_y,int width,int height,int dest_x, int dest_y);
extern void myxcpareaFrom(Display *display,Drawable srcDraw,GC gc,int src_x,int src_y,int width,int height,int dest_x, int dest_y);
extern void myxshmputimage(Display *display, GC gc, XImage *theimage, int src_x, int src_y, int dest_x, int dest_y, unsigned int width, unsigned int height);
extern void myxputimage(Display *display, GC gc, XImage *theimage, int src_x, int src_y, int dest_x, int dest_y, unsigned int width, unsigned int height);
#endif
