...
static int  best_period(real buffer[], int buflen, int pmin, int pmax);
static int  extract_pitch(void);
...
const real shzscale[SPORDER+1]=  /* Precomputed Scales for IIR coefficients */
...
const real shpscale[SPORDER+1]=  /* Precomputed Scales for FIR Coefficients */
...
static int  pitch_period;
static real pitch_tap;
static real pitch_gain;
...
void inv_filter(LDCELP_STATE *ldst,real input[])
{
  ...
  for(i=IDIM; i<SPORDER+IDIM; i++) mem1[i-IDIM] = mem1[i];
  for(i=0; i<IDIM; i++) mem1[SPORDER+i] = input[i];
  ...
  for(k=0; k<IDIM; k++)
  {
    ...
    for(j=1; j<=SPORDER; j++)
      tmp += mem1[SPORDER+k-j]*LDST(a10)[j];
    fil_mem[PMSIZE-NFRSZ+ip+k] = tmp;
  }
  if(ip == (NFRSZ-IDIM)) ip = 0;
  else ip += IDIM;
}
...
postfilter(real input[], real output[])
{
    ...
    static real
	ltemp[IDIM], 	/* Output of long term filter*/
	stemp[IDIM], 	/* Input of short term filter*/
    ...
    longterm(input, ltemp);
    shortterm(ltemp, stemp);

    /* Computed scale for gain preservation */

    new_gain = vec_abs(stemp);
    if (new_gain > 1.0)
    {
...
psf_adapter(real frame[])
{
    ...
    {
      if (pitch_tap > 1) pitch_tap = 1.0;
      if (pitch_tap < PPFTH) pitch_tap = 0.0;
...
void compute_sh_coeff(LDCELP_STATE *ldst)
{
    ...
    for(i=1; i<=SPORDER; i++) {
	shzcoef[i] = shzscale[i]*LDST(a10)[i];
	shpcoef[i] = shpscale[i]*LDST(a10)[i];
    }
    tiltz = TILTF * LDST(k10);
}
...
static int extract_pitch(void)
{
	...
	permax;
    real best_corr=-(BIG), best_old_corr=-(BIG), tap0=0.0, tap1=0.0;
    ...
    /* Filter and  decimate input */
    {
	  ...
	  tmp = fil_mem[PMSIZE-NFRSZ+k] - A1 * FIL_OUT_MEM(2)
	                - A2 * FIL_OUT_MEM(1) - A3 * FIL_OUT_MEM(0);
	  decim_phase++;
	  if (decim_phase == 4) {
