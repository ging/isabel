...
static int
lame_decode1_headersB_clipchoice(MPSTR * mp,
...
int
lame_decode1_headersB(MPSTR * mp,unsigned char *buffer,
                      int len,
                      ...
                      int *enc_delay, int *enc_padding)
{
    ...
    return lame_decode1_headersB_clipchoice(mp,buffer, len, (char *) pcm_l, (char *) pcm_r, mp3data,
...
int
lame_decode1_unclipped(MPSTR * mp,unsigned char *buffer, int len, sample_t pcm_l[], sample_t pcm_r[])
{
...
int
lame_decode1_headers(MPSTR * mp,unsigned char *buffer,
                     int len, short pcm_l[], short pcm_r[], mp3data_struct * mp3data)
{
    int     enc_delay, enc_padding;
    return lame_decode1_headersB(mp,buffer, len, pcm_l, pcm_r, mp3data, &enc_delay, &enc_padding);
}


int
lame_decode1(MPSTR * mp,unsigned char *buffer, int len, short pcm_l[], short pcm_r[])
{
    mp3data_struct mp3data;

    return lame_decode1_headers(mp,buffer, len, pcm_l, pcm_r, &mp3data);
}
...
int
lame_decode_headers(MPSTR * mp,unsigned char *buffer,
                    int len, short pcm_l[], short pcm_r[], mp3data_struct * mp3data)
{
    int     ret;
    ...
    for (;;) {
        switch (ret = lame_decode1_headers(mp,buffer, len, pcm_l + totsize, pcm_r + totsize, mp3data)) {
        case -1:
            return ret;
        case 0:
            return totsize;
...
int
lame_decode(MPSTR * mp,unsigned char *buffer, int len, short pcm_l[], short pcm_r[])
{
    mp3data_struct mp3data;

    return lame_decode_headers(mp,buffer, len, pcm_l, pcm_r, &mp3data);
}
