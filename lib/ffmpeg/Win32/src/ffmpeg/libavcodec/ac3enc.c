...
/* prototypes for functions in ac3enc_fixed.c and ac3enc_float.c */
 ...
 av_cold void mdct_end(AC3MDCTContext *mdct);

 av_cold int mdct_init(AVCodecContext *avctx, AC3MDCTContext *mdct,
                             int nbits);

 void mdct512(AC3MDCTContext *mdct, CoefType *out, SampleType *in);

 void apply_window(DSPContext *dsp, SampleType *output, const SampleType *input,
                         const SampleType *window, int n);

 int normalize_samples(AC3EncodeContext *s);

 void scale_coefficients(AC3EncodeContext *s);
