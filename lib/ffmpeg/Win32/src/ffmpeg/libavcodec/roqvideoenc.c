    ...
    for (i=0; i<enc->height; i+=blocksize)
        for (j=0; j<enc->width; j+=blocksize) {
            ...
            lowestdiff = eval_motion_dist(enc, j, i, (motion_vect) {{0,0}},
                                          blocksize);
#else
            motion_vect motVect= {{0, 0}};
            lowestdiff = eval_motion_dist(enc, j, i, motVect,
                                          blocksize);
...
static struct AVRational[] supFrameRates={{30,1}, {0,0}};
static enum PixelFormat PFs[]={PIX_FMT_YUV444P, PIX_FMT_NONE};
