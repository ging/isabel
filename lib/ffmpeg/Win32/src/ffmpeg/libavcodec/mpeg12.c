...
#if !CONFIG_VDPAU
// we provide "empty" implementations, to shut VS 2008 linker
// complaining about "unresolved external symbol"
...
int  ff_xvmc_field_start(MpegEncContext*s, AVCodecContext *avctx) { return -1; };
...
#else
                    AVRational size= {s ->width, s->height};
                    s->avctx->sample_aspect_ratio=
                        av_div_q(
                        ff_mpeg2_aspect[s->aspect_ratio_info],
                        size
...
#else
                    AVRational size= {s1->pan_scan.width, s1->pan_scan.height};
                    s->avctx->sample_aspect_ratio=
                        av_div_q(
                        ff_mpeg2_aspect[s->aspect_ratio_info],
                        size
