...
#else
    SoftFloat __sfRetVal;
    if(t<0) { __sfRetVal.exp = b.exp; __sfRetVal.mant= b.mant + (a.mant >> (-t)); }
    else    { __sfRetVal.exp = a.exp; __sfRetVal.mant= a.mant + (b.mant >>   t ); }
    return av_normalize1_sf(__sfRetVal);
#endif
}
...
#else
    SoftFloat retVal= {b.exp, -b.mant};
    return av_add_sf(a, retVal);
#endif
}
...
#else
    SoftFloat retVal= {ONE_BITS-frac_bits, v};
    return av_normalize1_sf(retVal);
#endif
}
