...
#########################################################################
#
# $Id: wb_rectangle.tcl 20206 2010-04-08 10:55:00Z gabriel $ 
#
# Description
#
#   Esta clase esta asociada a la figura de tipo rectangulo
...
class Rectangle {
   ...
   constructor {pag fig coords color outline stipple width} {}
   ...
   private method _show_filled {color}
   private method _change_info_pattern {pattern}
}
...
#                             CONSTRUCTOR
# ----------------------------------------------------------------------
body Rectangle::constructor {pag fig coords color outline \
  stipple width} {
   ...
   set page($pag,id) [eval $canvas create rectangle $coords \
...
body Rectangle::show_info {} {

   global normalbg entry_colour dir_iconos page
   ...
   radiobutton $f.solid  -width 2 -height 1 -variable pattern -value solid \
          -bg white -indicatoron 0  -command \
   ...
   for {set j 0} {$j < [expr [llength $_coords] -1]} {incr j 2} {
	  ...
	  trace_busy "Rectangle::show_info:: COORDS X,Y de $j son \
                $_posi(x,$j) $_posi(y,$j)"
   }
...
body Rectangle::_change_info_pattern {pattern} {
   ...
   if {$pattern == ""} { set _fig_modo "hueca"}
   set _fig_pattern $pattern
}



body Rectangle::_show_filled {color} {
   global fill
   if {$fill} {
...
body Rectangle::_show_calculate_coords {} { 
...
body Rectangle::_show_calculate_tags {} {
   ...
   if {$_fig_pattern == "none"} {
      ...
      set new_options [list -outline $_fig_borde \
          ...
          -stipple @$dir_iconos$_fig_pattern]
   }
   return $new_options
}
...
body Rectangle::copy {replicated} {
   ...
   create_figure ${mypag}fig$replicated \
      $mypag $replicated rectangle $newCoords  \"$color\" \
...
body Rectangle::move_points {prop posx posy} {
   ...
   switch $prop {
      punto1 {
         if {([lindex $coords 2] > $posx) && ([lindex $coords 3] > $posy)} {
