...
/////////////////////////////////////////////////////////////////////////
//
// $Id: controlProtocol.cc 20759 2010-07-05 10:30:36Z gabriel $
//
/////////////////////////////////////////////////////////////////////////
...
binding_t<mcu_InterfaceDef_t> mcu_methodBinding[]=
{
    ...
    { "new_nat_participant",    &mcu_InterfaceDef_t::new_nat_participant   },
...
mcu_InterfaceDef_t::new_participant(int argc, char **argv)
{
    ...
    inetAddr_t *addr;
    inetAddr_t *groupAddr = NULL;

    char *index = strchr(IP,'/');
    if (index == NULL)
    {
        addr = new inetAddr_t(IP, NULL, SOCK_DGRAM);
    }
    else
    {
        strncpy(GR,IP,strlen(IP)-strlen(index));
        GR[strlen(IP)-strlen(index)] = NULL;
        strcpy(IP,(index +1));
        addr = new inetAddr_t(IP, NULL, SOCK_DGRAM);
        groupAddr = new inetAddr_t(GR, NULL, SOCK_DGRAM);
    }
    ...
    HRESULT hr =
        APP->newParticipant(n_session, *addr, groupAddr, portList, &ID_part);
...
char const *
mcu_InterfaceDef_t::new_nat_participant(int argc, char **argv)
{
    static char result[4096];
    if (argc < 3)
    {
        return "ERROR: Syntax:: new_nat_participant(n_session,rx_port1[,rx_port2,...])\n";
    }
    ...
    for (u16 i= 2; i < argc; i++)
    {
        u16 port = atoi(argv[i]);
        portList << port;
    }

    HRESULT hr = APP->newNatParticipant(n_session, portList, &ID_part);
...
mcu_InterfaceDef_t::receive(int argc, char**argv)
{
    ...
    HRESULT hr = APP->receive(n_session, ID_part_rx, ID_part_tx, PT);
...
mcu_InterfaceDef_t::receive_video_mode(int argc, char**argv)
{
    ...
    else if (strcmp("SSRC_CHANGE_MODE",argv[3]) == 0)
    {
        mode = VIDEO_SSRC_CHANGE_MODE;
    }
    else if (strcmp("TRANS_MODE",argv[3]) == 0)
    {
...
mcu_InterfaceDef_t::receive_audio_mode(int argc, char**argv)
{
    ...
    if (argc < 4 || argc > 7)
    {
        return "ERROR: Syntax:: receive_audio_mode(n_session,id_part,mode[,PT,SL,SSRC])\n";
    }
    ...
    int SSRC = argc == 7 ? atoi(argv[6]) : 0;

    HRESULT hr = APP->receiveAudioMode(n_session, ID_part, mode, PT, SL, SSRC);
    sprintf(result,"%s\n", getMessageByCode(hr));
    return result;
}
...
mcu_InterfaceDef_t::receive_audio(int argc, char **argv)
{
    ...
    if (argc < 4 || argc > 5)
    {
        return "ERROR: Syntax:: receive_audio(n_session,id_part_rx,id_part_tx,[SSRC])\n";
    }

    u16 n_session  = atoi(argv[1]);
    u16 ID_part_rx = atoi(argv[2]);
    u16 ID_part_tx = atoi(argv[3]);
    int SSRC = (argc == 5) ? atoi(argv[4]) : 0;

    HRESULT hr = APP->receiveAudio(n_session,
                                   ID_part_rx,
                                   ID_part_tx,
                                   SSRC
                                  );
...
mcu_InterfaceDef_t::configure_participant_audio(int argc, char**argv)
{
    ...
    HRESULT hr = APP->configAudioPart(n_session,
                                      ID_part,
                                      oldPT,
                                      newPT
                                     );
