...
/////////////////////////////////////////////////////////////////////////
//
// $Id: RTCPRecv.cc 20759 2010-07-05 10:30:36Z gabriel $
//
/////////////////////////////////////////////////////////////////////////
...
RTCPRecv_t::IOReady(io_ref &io)
{
    ...
    u16 pos = getPosition(*socket->temporaryClientAddr);
...
double
RTCPRecv_t::getLosses(int type, inetAddr_t const &addr)
{
    // get reportBlockList
    double losses = 0.0;
    u16 pos = getPosition(addr);
    u16 j = 0;
...
u16
RTCPRecv_t::getPosition(inetAddr_t const &addr)
{
    // hash table
    u16 position = getIndex(addr);

    // check duplicates
    while (addrReportArr[position])
    {
        if (addr.equalIPAddr(addrReportArr[position]->addr))
        {
            break;
        }
...
HRESULT
RTCPRecv_t::addReceiver(inetAddr_t const &addr, int PT)
{
    // get position
    u16 pos = getPosition(addr);
    if ( ! addrReportArr[pos])
...
HRESULT
RTCPRecv_t::deleteReceiver(inetAddr_t const &addr)
{
    u16 pos = getPosition(addr);
    if (addrReportArr[pos])
    {
...
// ADD/DELETE RTCP receiver
HRESULT
RTCPDemux_t::addReceiver(inetAddr_t const &addr,
                         int PT,
                         u16 localRTCPport
                        )
{
    ...
    return RTCPRecv->addReceiver(addr, PT);
}

HRESULT
RTCPDemux_t::addReceiver(int PT, u16 localRTCPport)
{
    RTCPRecv_t *RTCPRecv = NULL;
    ...
    return S_OK; // RTCPRecv->addReceiver(IP, PT);
}

HRESULT
RTCPDemux_t::deleteReceiver(inetAddr_t const &addr, u16 localRTCPport)
{
    if (localRTCPport)
    {
        for (int i= 0; i < RTCPRecvArray.size(); i++)
...
//      -- TOTAL_LOSSES
double
RTCPDemux_t::getLosses(int type, inetAddr_t const &addr)
{
    double losses = 0.0;

    for (int i= 0; i < RTCPRecvArray.size(); i++)
    {
          if ((losses = RTCPRecvArray.elementAt(i)->getLosses(type,addr)) >= 0.0)
          {
              return losses;
          }
    }
