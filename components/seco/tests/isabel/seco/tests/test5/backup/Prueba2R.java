 ...
 * @author ibau
 */
public class Prueba2R extends Thread {

	private static final int SECO_PORT = 53023;
	
	private Prueba2R(String arg){
		...
		try {
				...
				net = Network.createNetwork("Prueba2R", "localhost", SECO_PORT,
						new JavaMarshaller());
		...
		ListenerTemporizador2 tempo = new ListenerTemporizador2(clientes);
		jDispatcher.addDestiny(TmpMsg.class,tempo);
		
		
		try {
			net.joinGroup("prueba2");
			mylogger.info("Me he unido al grupo prueba2");
