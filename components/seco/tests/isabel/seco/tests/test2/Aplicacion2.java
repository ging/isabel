 ...
 */
public class Aplicacion2 extends Thread{

	private static final int SECO_PORT = 53023;
	
	public void run() {
		final Logger mylogger = Logger.getLogger("isabel.seco.tests.test2");
		JavaDispatcher jDispatcher = JavaDispatcher.getDispatcher("app2");

		Network net = null;
		try {
				net = Network.createNetwork("app2", "localhost", SECO_PORT,
						new JavaMarshaller());
		...
		try {
			...
			mylogger.info("Me he unido al grupo local del root: pregunta");
		...
		jDispatcher.addDestiny(SumaMsg.class, new JavaDispatcherListener() {
			public void processMessage(JavaMessage msg) {
				...
				int sol = s.s1 + s.s2;
				try {
					fn.sendClient(s.senderId, new RespuestaMsg("Aplicación 2", suma, sol));
		...
		jDispatcher.addDestiny(HelloMsg.class,
				new JavaDispatcherListener() {
					public void processMessage(JavaMessage msg) {
						...
						fn.sendClient(h.id, new HelloMsg("Aplicacion2",fn.getClientID()));
						} catch (Exception e) {
