...
public class RegisterAgent implements Runnable, TransactionClientListener
{
   ...
   public RegisterAgent(SipProvider sip_provider, String target_url, String contact_url, String nickname, String username, String realm, String passwd, RegisterAgentListener listener)
   {  init(sip_provider,target_url,contact_url,listener);
      // authentication
   ...
   public void publish(int expire_time)
   {
	  if (expire_time>0) this.expire_time=expire_time;
      Message req=MessageFactory.createPublishRequest(sip_provider,target,contact,contact,nickname);
      req.setExpiresHeader(new ExpiresHeader(expire_time));
      TransactionClient t=new TransactionClient(sip_provider,req,this);
      t.request(); 
   }
   ...
   {  if (transaction.getTransactionMethod().equals(SipMethods.REGISTER) ||
		  transaction.getTransactionMethod().equals(SipMethods.PUBLISH))
      {  StatusLine status=resp.getStatusLine();
         ...
         {  attempts++;
            ...
            AuthorizationHeader ah=(new DigestAuthentication(transaction.getTransactionMethod(),req.getRequestLine().getAddress().toString(),wah,qop,null,username,passwd)).getAuthorizationHeader();
            req.setAuthorizationHeader(ah);
            TransactionClient t=new TransactionClient(sip_provider,req,this);
            t.request();
         }else if (code==407 && attempts<MAX_ATTEMPTS && resp.hasProxyAuthenticateHeader() && resp.getProxyAuthenticateHeader().getRealmParam().equalsIgnoreCase(realm))
         {  attempts++;
            Message req=transaction.getRequestMessage();
            req.setCSeqHeader(req.getCSeqHeader().incSequenceNumber());
            ...
            //printLog("DEBUG: qop-options: "+qop_options,LogLevel.MEDIUM);
            qop=(qop_options!=null)? "auth" : null;
            AuthorizationHeader ah=(new DigestAuthentication(transaction.getTransactionMethod(),req.getRequestLine().getAddress().toString(),pah,qop,null,username,passwd)).getAuthorizationHeader();
            req.setAuthorizationHeader(ah);
            TransactionClient t=new TransactionClient(sip_provider,req,this);
            t.request();
         }
