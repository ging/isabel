...
public abstract class BaseMessageFactory
{
   ...
   public static Message createRequest(String method, SipURL request_uri, NameAddress to, NameAddress from, NameAddress contact, String proto, String via_addr, int host_port, boolean rport, String call_id, long cseq, String local_tag, String remote_tag, String branch,String body_type, String body)
   {  Message req=new Message();
      ...
      req.setAllowHeader(new AllowHeader(" INVITE, ACK, CANCEL, BYE, OPTIONS, REFER, SUBSCRIBE, NOTIFY, MESSAGE"));
      //optional headers:
      if (contact!=null)
   ...
   {  String via_addr=sip_provider.getViaAddress();
      ...
      return createRequest(method,request_uri,to,from,contact,proto,via_addr,host_port,rport,call_id,cseq,local_tag,remote_tag,branch,"application/sdp",body);
   }
   ...
   {  NameAddress to=dialog.getRemoteName();
      ...
      //String branch=SipStack.pickBranch();
      Message req=createRequest(method,request_uri,to,from,contact,proto,via_addr,host_port,rport,call_id,cseq,local_tag,remote_tag,null,"application/sdp",body);
   ...
   /** Creates a new PUBLISH request out of any pre-existing dialogs.
    * @see #createRequest(String,SipURL,NameAddress,NameAddress,NameAddress,String,String,int,boolean,String,long,String,String,String,String) */
  public static Message createPublishRequest(SipProvider sip_provider, NameAddress to, NameAddress from, NameAddress contact, String nickname)
  {  
      ...
      "<presence xmlns=\"urn:ietf:params:xml:ns:pidf\"\r\n" +
                "entity=\"" + nickname + "\">\r\n" +
                "<tuple id=\"isabel-gateway\">\r\n" +
                    "<status><basic>open</basic></status>\r\n" +
                    "<note>online</note>\r\n" +
                    "<contact priority=\"1\">" + to.getAddress().toString() + "</contact>\r\n" +
                "</tuple>\r\n" +      
      "</presence>\r\n";
      
      Message req=createRequest(SipMethods.PUBLISH,registrar,to,from,contact,proto,via_addr,host_port,rport,call_id,cseq,local_tag,null,null,"application/pidf+xml",xml);
      req.setRequestLine(new RequestLine(SipMethods.PUBLISH,to.getAddress().toString()));      
      req.setEventHeader(new EventHeader("presence"));
      req.setToHeader(new ToHeader(to));
      req.setFromHeader(new FromHeader(to));
      return req;
  }
   ...
   {  SipURL request_uri=method.getRequestLine().getAddress();
      ...
      Message ack=createRequest(SipMethods.ACK,request_uri,to.getNameAddress(),from.getNameAddress(),contact,proto,via_addr,host_port,rport,method.getCallIdHeader().getCallId(),method.getCSeqHeader().getSequenceNumber(),from.getParameter("tag"),to.getParameter("tag"),branch,null,null);
   ...
   {  ToHeader to=method.getToHeader();
      ...
      String branch=method.getViaHeader().getBranch();
      return createRequest(SipMethods.CANCEL,request_uri,to.getNameAddress(),from.getNameAddress(),contact,proto,host_addr,host_port,rport,method.getCallIdHeader().getCallId(),method.getCSeqHeader().getSequenceNumber(),from.getParameter("tag"),to.getParameter("tag"),branch,null,null);
   }
   ...
   {  SipURL to_url=to.getAddress();
      ...
      String local_tag=SipProvider.pickTag();
      //String branch=SipStack.pickBranch();
      Message req=createRequest(SipMethods.REGISTER,registrar,to,from,contact,proto,via_addr,host_port,rport,call_id,cseq,local_tag,null,null,null,null);
