...
public class FLVWriter implements ITagWriter {
	...
	/**
	 * OutputStream used when there is no file
	 */
	...
	public FLVWriter (OutputStream os){
		this.os = os;
	...
	private byte[] getByte(ByteBuffer buf){
...
//		IOUtils.writeMediumInt(out, tag.getTimestamp() + offset);
		if (tag.getDataType() == ITag.TYPE_AUDIO|| tag.getDataType() == ITag.TYPE_VIDEO){
		
			if (timeStart == -1){
				timeStart = System.currentTimeMillis();
				IOUtils.writeMediumInt(out, 0);			
			}else{
				int ts= (int)(System.currentTimeMillis() - timeStart);
				IOUtils.writeMediumInt(out, ts);			
			}
		...
		if (noFile){
			bytesWritten += out.buf().limit() - out.buf().position();
			os.write(getByte(out));
			os.flush();
			
			bodyBuf = tag.getBody();
			bytesWritten += bodyBuf.limit() - bodyBuf.position();
			os.write(getByte(bodyBuf));
			os.flush();
		}else{
